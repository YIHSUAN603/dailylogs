...
/** 要求 AI 嚴格輸出的「專案(H2) > 功能(H3) > 面向條列」格式說明（對齊團隊 Google Doc：專案為主標、功能為副標，事項標進度 emoji） */
const FORMAT_RULE = `請「只」輸出以「專案（H2 主標）> 功能（H3 副標）> 面向條列」為結構的日報，格式如下，不要任何前言、結語或其他說明：
...
規則：
- 可以有多個專案，每個以「## 」開頭當主標；每個專案底下可有多個功能，各以「### 」開頭當副標。
- 每個功能底下固定三個面向，各自是一個以「- 」開頭的項目，依序為：完成、進行中、問題／待辦。
- 各面向的實際事項用「縮排 4 個空白的 - 」當子項目條列；該面向沒有內容就在其下寫「    - 無」。
- 每個事項句尾標進度：完成用 ✅、進行中用 ⏳、卡關或有問題用 ❌（「問題／待辦」中的純待辦事項可不標 emoji）。
- 需要補充細節時，事項底下可再往下縮排一層（8 個空白）條列。
